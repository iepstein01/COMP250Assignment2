...
public class MyBigInteger  implements Comparable<MyBigInteger> {   //  We will cover what "implements Comparable" means later
	...
	public MyBigInteger dividedBy( MyBigInteger  divisor ) throws Exception{
		...
		{
			System.out.println("ERROR: bases must be the same in division");
			throw new Exception();
		}
		else if(divisor.compareTo(new MyBigInteger(0,dividend.base))==0)
			return null;
		else
		{
			int result = dividend.compareTo(divisor);
			if(result == 0)
				return new MyBigInteger(1,dividend.base);
			else if(result == -1)
				return new MyBigInteger(0,dividend.base);
			if(result == 1)
			{
				MyBigInteger smallerInt = makeSmallerBigInteger(dividend, divisor.coefficients.size());
				MyBigInteger temp;
				int i=dividend.coefficients.size()-divisor.coefficients.size();
				while (i >= 0)
				{
					smallerInt.coefficients.add(0,dividend.coefficients.get(i));
					result=smallerInt.compareTo(divisor);
					if(result == 1 || result==0){
						if(smallerInt.coefficients.get(smallerInt.coefficients.size()-1)==0)
							smallerInt.coefficients.remove(smallerInt.coefficients.size()-1);
						temp=smallerInt.slowdividedBy(divisor);
						for(int j=temp.coefficients.size()-1;j>=0;j--)
							quotient.coefficients.add(0,temp.coefficients.get(j));
						smallerInt=smallerInt.minus(divisor.slowTimes(temp));
						i-=1;
					}
					else if(result == -1){
						if(quotient.coefficients.size()!=0)
	...
	public MyBigInteger convert(int newBase) throws Exception {
		...
		if(newBase >=2 && newBase <= 10) {
			if (newBase == remainder.base)
				result = remainder;
			else {
				int index = remainder.coefficients.size() - 1;
				MyBigInteger temp = new MyBigInteger(remainder.coefficients.get(index), newBase);
				MyBigInteger base = new MyBigInteger(remainder.base, newBase);
				while (index > 0) {
					temp = (temp.slowTimes(base)).plus(new MyBigInteger(remainder.coefficients.get(index - 1), newBase));
					index -= 1;
				}
				result = temp;
			}
	...
	public ArrayList<MyBigInteger>  primeFactors() throws Exception {
		...
		MyBigInteger zero = new MyBigInteger(0,copy.base);
		MyBigInteger one = new MyBigInteger(1,copy.base);
		MyBigInteger two = new MyBigInteger(2, copy.base);
		MyBigInteger three = new MyBigInteger(3, copy.base);
		while(copy.mod(two).compareTo(zero)==0 && copy.compareTo(one)!=0){
			factors.add(two);
			copy=copy.dividedBy(two);
		}
		for (MyBigInteger i = three; i.times(i).compareTo(copy) <= 0 && copy.compareTo(one)!=0; i = i.plus(two)) {
			while (copy.mod(i).compareTo(zero)==0) {
				factors.add(i);
				copy = copy.dividedBy(i);
				if(factors.size()>0) {
					for (int j=0;j<factors.size();j++) {
						while (copy.mod(factors.get(j)).compareTo(zero) == 0) {
							factors.add(factors.indexOf(factors.get(j)),factors.get(j));
							copy = copy.dividedBy(factors.get(j));
						}
					}
				}
			}
		}
		if (copy.compareTo(one) >=1)
			factors.add(copy);
		//  ADD YOUR CODE ABOVE HERE
	...
	*/
	...
	/*
	*   This method truncates 'this' to the size of the divisor-1
	*/
	private MyBigInteger makeSmallerBigInteger(MyBigInteger big, int length)
	{
		MyBigInteger newInt = new MyBigInteger(big.base);
		for(int i=big.coefficients.size()-1;i>=big.coefficients.size()-length+1;i--)
		{
			newInt.coefficients.add(0,big.coefficients.get(i));
		}
		return newInt;
	}
